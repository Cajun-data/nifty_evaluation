...
class EvaluateRules:
    ...
    def evaluate_pairs(self, bool_matrix, binarized_labels) -> list:
        ...
        if self._n_pos == 0 and self._n_neg == 0:
            return np.abs(0)

        block_size = max(1, self._PAIR_BLOCK_ELEMENTS // max(1, bool_matrix.shape[1]))
        final_scores = np.empty(bool_matrix.shape[0], dtype=np.float64)
        for start in range(0, bool_matrix.shape[0], block_size):
            block = bool_matrix[start:start + block_size]
            # Keep the original label dtype for wide integer accumulation.
            # Label permutations are generated by the caller, never per block.
            TP = block.dot(binarized_labels)
            FP = block.dot(inverse)
            TP_prop = TP / self._n_pos if self._n_pos > 0 else 0
            FP_prop = FP / self._n_neg if self._n_neg > 0 else 0
            final_scores[start:start + block_size] = np.abs(TP_prop - FP_prop)
